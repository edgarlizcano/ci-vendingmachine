...
stdin.addListener("data", function (d) {
    ...
    switch (cmd) {
        case "1":
            ...
            controlTime(6000, 7, 5, function (err) {
                if (err) {
        ...
        case "2":
            ...
            controlTime(4000, 7, 5, function (err) {
                if (err) {
                    console.log("Posible atasco " + err);
                }
                else {
        ...
        case "3":
            ...
            controlTime(10000, 7, 1, function (err) {
                if (err) {
                    console.log("Posible atasco " + err);
                }
                else {
        ...
        case "4":
            ...
            controlTime(13000, 7, 1, function (err) {
                if (err) {
                    console.log("Posible atasco " + err);
                }
                else {
        ...
        case "5":
            console.log("Tiempo de 10000 6 pisos llega bien");
            controlTime(10000, 1, 7, function (err) {
                if (err) {
                    console.log("Posible atasco " + err);
                }
                else {
        ...
        case "6":
            console.log("Tiempo de 13000 6 pisos atasco");
            controlTime(13000, 1, 7, function (err) {
                if (err) {
                    console.log("Posible atasco " + err);
                }
                else {
...
function controlTime(dato, location, goingto, callback) {
    var nPisos = location - goingto;
    var time = 0;
    var countTime = 0;
    if (nPisos < 0) {
        nPisos = nPisos * -1;
...
var atasco = false;
var intentos = 0;
function controlAtasco(state, callback) {
    atasco = true;
    intentos++;
    if (intentos > 2) {
